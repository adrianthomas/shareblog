...
test("folds every search destination into one neutral disclosure", () => {
  ...
  assert.match(html, /<summary>Search for this book<\/summary>/);
  assert.match(html, />OverDrive <span/);
  assert.match(html, />Standard Ebooks <span/);
  assert.doesNotMatch(html, /Borrow digitally|Read free/);
