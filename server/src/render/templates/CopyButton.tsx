...
// Fediverse handle (@subdomain@host) — rendered as a plain word ("Fediverse")
// next to the RSS link in Layout.tsx rather than the handle itself, since
// the full @handle@host was too long to sit comfortably on one line next to
// "Follow RSS". There's no universal "click to follow" across federated
// servers anyway, so copying the handle for a visitor to paste into their
// own instance's search bar is the point — the tooltip/aria-label says so.
export function CopyHandleButton({
  ...
  const title = t(locale, "copyFediverseHandle");
  return (
    <button
      type="button"
      className={`rss-link link-btn-reset${className ? ` ${className}` : ""}`}
      data-copy="text"
